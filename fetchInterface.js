...
async function main() {
    ...
    if (moduleCode === "list") {
        ...
        console.error(
            "❌ กรุณาระบุโมดูลที่ถูกต้อง เช่น: HPAI002, HPAI003, DBTEST, list",
        );
        process.exit(0);
        ...
        if (!otype || !infty) {
            console.error(
                "❌ สำหรับ insert กรุณาระบุ IV_OTYPE และ IV_INFTY เช่่น : npm run insert -- O 1000",
            );
            process.exit(0);
        } else if (infty === "1000") {
            const headers = {
        ...
        try {
            ...
            const [rows] = await conn.query(
                `SELECT * FROM org_chart ORDER BY dropdown_type LIMIT 100;`,
            );
        ...
        try {
            ...
            const outputPath = path.join(outputDir, `positions_v${otype}.json`);
            fs.writeFileSync(outputPath, JSON.stringify(result, null, 2), "utf8");
            console.log(`✅ บันทึกข้อมูล positions_v${otype}.json สำเร็จ`);
            process.exit(0);
        } catch (err) {
        ...
        if (moduleCode === "HPAI002") {
            if (!otype || !infty) {
                console.error(
                    "❌ สำหรับ HPAI002 กรุณาระบุ IV_OTYPE และ IV_INFTY เช่น: node fetchInterface.js HPAI002 O 1000",
                );
                process.exit(0);
            }
        ...
        try {
            ...
            const items =
                response.data.ET_HRP_DATA?.item ?? response.data.ET_INFTY_DATA?.item;
